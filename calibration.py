...
class Exponential_Model:
    ...
    def fit_calibration_curve(self):
        self.popt, _ = curve_fit(self.exponential_model, self.x_data, self.y_data)

    def plot_calibration_curve(self,xlabel,ylabel,title):
        ...
        ax1.plot(self.x_data, self.linear_model(self.x_data, *self.popt), 'k--', label="Anpassung")
        ax1.legend()
...
class Linear_Regression_Model:
    ...
    def fit_calibration_curve(self):
        self.popt, _ = curve_fit(self.linear_model, self.x_data, self.y_data)
    ...
    def calculate_linear_statistics(self):
        residuals = self.y_data - self.linear_model(self.x_data, *self.popt)
        S = np.std(residuals)
        mean_concentration = np.mean(self.x_data)
        RSD = (S / mean_concentration) * 100
    ...
    def calculate_r_squared(self):
        ...
        # Berechnung von R² für die nicht-lineare Kalibration
        fitted_values = self.linear_model(self.x_data, *self.popt)
        residuals = self.y_data - fitted_values
...
if __name__ == "__main__":
    ...
    calibration = Linear_Regression_Model.import_data_from_excel(excel_file, sheet_name)
    calibration.fit_calibration_curve()

    LOD = calibration.calculate_LOD()
    LOQ = calibration.calculate_LOQ()
    S, RSD = calibration.calculate_linear_statistics()

    print(f"LOD: {LOD}")
    print(f"LOQ: {LOQ}")
    print(f"Standardabweichung der Regression (S): {S}")
    print(f"Relative Standardabweichung der Regression (RSD): {RSD}%")

    calibration.plot_calibration_curve(xlabel="Konzentration [µg/L]",ylabel="Peakarea",title="Test der Kalibration")
